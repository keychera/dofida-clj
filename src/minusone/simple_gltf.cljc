...
(defn init-fn [world game]
  (-> world
      (esse ::pmx-shader #::shader{:program-data (shader/create-program game vert frag)})
      (esse ::simpleskin
...
(defn create-joint-mats-arr ^floats [skin transform-db inv-bind-mats]
  (let [transform-db (update-in transform-db [3 :global-transform]
                                (fn [bone]
                                  (->> bone
...
(-> hmm (get 3) :global-transform vec)
