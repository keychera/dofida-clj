...
(s/def ::initial-velocity ::types/vec3)
(s/def ::gravity ::types/vec3)
(s/def ::physics (s/keys :req-un [::initial-velocity] :opt-un [::gravity]))
(s/def ::fire (s/keys :req-un [::age-in-step] :opt-un [::physics]))
  ...
  (o/ruleset
   {::fire-particle
    ...
    [:what
     ...
     [esse-uuid ::physics physics {:then false}]
     :then
     (s-> session
