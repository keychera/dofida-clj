...
;; consider that feeling inside of you, stop, and look at your surrounding
;; this person already have all they need, restrain on giving more, discreetly

(defn make-IK-solver2 [pole]
  (fn [root-t mid-t end-t target-t]
    (let [origin-v     (m/- end-t root-t)
          ...
          cos-beta     (/ (+ (* AB AB) (* BC BC) (* -1.0 AC AC))
                          (* 2.0 AB BC))
          beta         (m-ext/clamped-acos cos-beta)
          root-IK      (cond-> (q/quat-from-axis-angle axis (- alpha))
                         flip? m/conjugate)
          mid-IK       (cond-> (q/quat-from-axis-angle axis (- Math/PI beta))
                         flip? m/conjugate)]
      [(m/* root-angle root-IK) mid-IK])))

(defn IK-transducer1
  "transducer assumes input conform to :minustwo.gl.geom/node+transform"
  ([a b c target] (IK-transducer1 a b c target solve-IK1))
  ([a b c target IK-solver]
   (fn [rf]
     (let [fa (volatile! nil)
           fb (volatile! nil)
           fc (volatile! nil)]
       (fn
         ...
         ([result]
          (let [fa @fa fb @fb fc @fc
                ...
                [root-IK mid-IK] (IK-solver root-t mid-t end-t target)
                result (-> result
